param(
    [string]$CaseFile = (Join-Path $PSScriptRoot 'day04-hallucination-bias-cases.json'),
    [string]$OutputFile = (Join-Path (Split-Path $PSScriptRoot -Parent) 'reports\day-04-hallucination-run.jsonl'),
...
foreach ($case in @($cases)) {
    ...
    $stopwatch.Stop()
    $record = [ordered]@{
        Id = $case.Id; Category = $case.Category; Objective = $case.Objective
    ...
    [pscustomobject]@{ Id = $case.Id; Category = $case.Category; Seconds = $record.DurationSeconds; Characters = $record.Response.Length } | Format-Table -AutoSize | Out-String | Write-Host
}
